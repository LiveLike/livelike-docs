...
| **widget\_interacted** | When the user submits a response to a widget.        | `widget_type` (image prediction, text quiz, text\_poll)<br />`widget_state`<br />`question_type`<br />`question_name`<br />`option_index`<br />`option_submitted`<br />`is_correct`(true/false)<br />`ll_program_name`<br />`ll_program_id`                  | Captures user engagement with prediction widgets.       |
