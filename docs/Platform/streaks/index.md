...
**If my streak resets after I reaching a milestone, can I re-earn same milestone rewards?**<br />Whether a milestone reward is granted again after reset depends on whether **Reward on Reset** is enabled on that milestone. Future milestone rewards are only available if the user rebuilds their streak back to those lengths.
