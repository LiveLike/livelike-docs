...
Passing key-value pair as custom attribute in streak progress action applies to all target-based streaks.
...
**Note:** Setting milestone is **_optional_**, needed only when users need to be rewarded for reaching specific streak lengths.&#x20;
...
**If a streak resets, are milestone rewards lost?**<br />Rewards already granted at prior milestones are not revoked. Whether a milestone reward is granted again after reset depends on whether **Reward on Reset** is enabled on that milestone. Future milestone rewards are only available if the user rebuilds their streak back to those lengths.
